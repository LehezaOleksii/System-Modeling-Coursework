...
import oleksii.leheza.kpi.ms.v2.enums.*;
...
public class Server {
    ...
    private void chooseProcessType(Request request) {
        int secondBusyProcesses = 0;
        for (Process process : secondProcesses) {
            if (process.isBusy()) {
                secondBusyProcesses++;
            }
        }
        if (secondBusyProcesses + secondProcessQueue.size() >= 1 || request.getRequestType().equals(RequestType.C.name())) {
    ...
    public void printServerStatistic() {
        ...
        double secondMean = (-1) * (allSecondQueueSizeSum) / (firstQueueSizeList.size() * (currentTime - startTimeTracking));
